...
import {
  ...
  Mic, PenTool, TrendingUp, Send, Calendar, Award,
  Upload, Settings, LayoutDashboard, ChevronRight, Layers
} from 'lucide-react';
...
const faqs = [
  { q: "Do I need any technical skills to use AlignAI?", a: "None at all. AlignAI is built for teachers, not developers. If you can type a message, you can use every feature." },
  { q: "What file formats can I upload for lectures?", a: "You can upload MP3, MP4, WAV, PDF, PPTX, and DOCX files. AlignAI handles transcription and processing for all of them." },
  { q: "How does the auto grading work?", a: "For objective questions, AlignAI grades automatically with 100% accuracy. For descriptive answers, AI gives an initial grade that you can review and adjust." },
  { q: "Can students access the platform on mobile?", a: "Yes. AlignAI works fully on mobile browsers. A dedicated app is on the roadmap for launch." },
  { q: "Is my data secure?", a: "All data is encrypted in transit and at rest. You own your data completely and can export or delete it any time." },
  { q: "How many students can I add?", a: "Plans vary from individual tutors to institutions with thousands of students. Early access users get generous limits." },
];
...
function PreRegisterModal({ dark, onClose }: { dark: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus('loading');
    try {
      const payload = { service_id: 'YOUR_SERVICE_ID', template_id: 'YOUR_TEMPLATE_ID', user_id: 'YOUR_PUBLIC_KEY', template_params: { from_name: name, from_email: email, role, to_email: 'aadityadagr@gmail.com' } };
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-500 ${bg}`}>
      ...
      <header className={`sticky top-0 z-50 backdrop-blur-xl border-b ${dark ? "border-white/[0.06] bg-[#080c12]/80" : "border-gray-200/80 bg-white/80"}`}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {NAV_LINKS.map(n => (
              <a key={n.label} href={n.href} className={`text-sm font-medium transition-colors ${n.href === '/how-it-works' ? (dark ? 'text-white' : 'text-gray-900') : (dark ? "text-gray-400 hover:text-white" : "text-gray-600 hover:text-gray-900")}`}>{n.label}</a>
            ))}
          </nav>
      ...
      <section className="max-w-4xl mx-auto px-6 pt-20 pb-16 text-center">
        <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium mb-6 border ${dark ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-emerald-50 border-emerald-200 text-emerald-700"}`}>
          <Layers size={12} /> 6 step workflow
        </div>
        <h1 className={`text-4xl md:text-6xl font-extrabold tracking-tight leading-tight mb-5 ${text}`}>
          From setup to fully<br />
          <span className="bg-gradient-to-r from-purple-400 to-emerald-400 bg-clip-text text-transparent">automated in one day.</span>
        </h1>
        <p className={`text-lg max-w-2xl mx-auto ${muted}`}>
          AlignAI is designed to feel familiar immediately and become indispensable within hours. Here is exactly how it works.
        </p>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-6 pb-24">
        <div className="relative">
          {/* connector line */}
          <div className={`absolute left-8 top-8 bottom-8 w-0.5 hidden md:block ${dark ? "bg-white/[0.06]" : "bg-gray-100"}`} />
            ...
            {steps.map((s, i) => (
              <div key={i} className="relative flex gap-6 md:gap-10">
                ...
                <div className="relative z-10 flex-shrink-0 w-16 flex flex-col items-center">
                  <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ backgroundColor: s.color + '20' }}>
                    <s.icon size={24} style={{ color: s.color }} />
                  </div>
                </div>

                {/* content */}
                <div className={`flex-1 rounded-2xl border p-7 pb-8 mb-2 transition-all duration-300 hover:scale-[1.01] ${card}`} style={{ borderColor: s.color + '25' }}>
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div>
                      <span className="text-xs font-bold tracking-widest mb-1 block" style={{ color: s.color }}>STEP {s.number}</span>
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {s.details.map((d, j) => (
                      <li key={j} className={`flex items-start gap-2 text-xs ${muted}`}>
      ...
      <section className={`border-t ${dark ? "border-white/[0.06]" : "border-gray-100"}`}>
        <div className="max-w-3xl mx-auto px-6 py-24">
          <div className="text-center mb-14">
            <h2 className={`text-3xl md:text-4xl font-bold mb-3 ${text}`}>Common questions</h2>
            <p className={`${muted}`}>Everything you need to know before getting started.</p>
          </div>
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <div key={i} className={`rounded-2xl border transition-all duration-200 overflow-hidden ${card}`}>
                <button
                  onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  className={`w-full flex items-center justify-between gap-4 px-6 py-5 text-left ${text}`}
                >
                  <span className="font-semibold text-sm">{faq.q}</span>
                  <ChevronRight size={16} className={`shrink-0 transition-transform duration-200 ${openFaq === i ? 'rotate-90 text-purple-400' : muted}`} />
                </button>
                {openFaq === i && (
      ...
      <section className={`border-t ${dark ? "border-white/[0.06]" : "border-gray-100"}`}>
        <div className="max-w-3xl mx-auto px-6 py-24 text-center">
          <h2 className={`text-3xl md:text-5xl font-extrabold tracking-tight mb-5 ${text}`}>
            Ready to try it yourself?
          </h2>
          <p className={`text-lg mb-8 ${muted}`}>AlignAI is almost here. Register now and be first in line.</p>

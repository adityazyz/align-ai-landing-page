  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-500 ${bg}`}>
      ...
      <header className={`sticky top-0 z-50 backdrop-blur-xl border-b ${dark ? "border-white/[0.06] bg-[#080c12]/80" : "border-gray-200/80 bg-white/80"}`}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {NAV_LINKS.map(n => (
              <a key={n.label} href={n.href} className={`text-sm font-medium transition-colors ${n.href === '/features' ? (dark ? 'text-white' : 'text-gray-900') : (dark ? "text-gray-400 hover:text-white" : "text-gray-600 hover:text-gray-900")}`}>{n.label}</a>
            ))}
          </nav>
      ...
      <div className={`sticky top-[69px] z-40 border-b ${dark ? "border-white/[0.06] bg-[#080c12]/90" : "border-gray-100 bg-white/90"} backdrop-blur-lg`}>
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-center gap-2 overflow-x-auto scrollbar-hide">
          {featureGroups.map(f => (
            <a key={f.id} href={`#${f.id}`} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${dark ? "text-gray-400 hover:text-white hover:bg-white/10" : "text-gray-500 hover:text-gray-900 hover:bg-gray-100"}`} style={{ color: f.color }}>
              {f.badge}
            </a>
          ))}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-16 space-y-32">
        {featureGroups.map((fg, i) => (
          <section key={fg.id} id={fg.id} className={`grid md:grid-cols-2 gap-16 items-center ${i % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
            {/* text side */}
            <div className={i % 2 === 1 ? 'md:order-2' : ''}>
              <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-xs font-medium mb-5 border" style={{ backgroundColor: fg.color + '15', borderColor: fg.color + '30', color: fg.color }}>
                <fg.icon size={11} />
                {fg.badge}
              </div>
              <h2 className={`text-3xl md:text-4xl font-extrabold leading-tight mb-4 ${text}`}>{fg.headline}</h2>
            ...
            <div className={`${i % 2 === 1 ? 'md:order-1' : ''}`}>
              <div className={`rounded-2xl border p-8 ${card}`} style={{ borderColor: fg.color + '25' }}>
                <div className="text-xs font-medium mb-6" style={{ color: fg.color }}>{fg.badge} workflow</div>
                <div className="space-y-4">
                  {fg.visual.map((v, j) => (
                    <div key={j} className="flex items-center gap-4">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0" style={{ backgroundColor: fg.color + '20', color: fg.color }}>
                        {v.step}
                      </div>
                      <div className={`flex-1 flex items-center gap-3 p-3 rounded-xl ${dark ? "bg-white/[0.04]" : "bg-gray-50"}`}>
                        <v.icon size={15} style={{ color: fg.color }} />
                        <span className={`text-sm font-medium ${text}`}>{v.label}</span>
                      </div>
                      {j < fg.visual.length - 1 && (
                        <div className="absolute ml-4 mt-12 w-0.5 h-4 bg-white/10" />
                      )}
                    </div>
                  ))}
                </div>
